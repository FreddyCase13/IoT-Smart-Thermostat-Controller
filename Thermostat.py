...
class TemperatureMachine(StateMachine):
    ...
    def on_enter_heat(self):
        ## Evaluate current temperature against the setPoint and updateUI the moment the state changes.
        self.updateLights()
        if(DEBUG):
    ...
    def on_exit_heat(self):
        ## Ensure Red Light is off.
        redLight.off()

    def on_enter_cool(self):
        ## Evaluate current temperature against the setPoint and update UI the moment the state changes.
        self.updateLights()
        if(DEBUG):
    ...
    def on_exit_cool(self):
        ## Ensure blue light is turned off.
        blueLight.off()

    def on_enter_off(self):
        ## Ensure all hardware outputs are off when the system shuts down.
        redLight.off()
        blueLight.off()
        if(DEBUG):
    ...
    def processTempStateButton(self):
        ...
        ## Trigger the StateMachine to transition to the next valid state
        ## defined in the cycle. (off, heat or cool).
        self.send("cycle")
    ...
    def processTempIncButton(self):
        ...
        ## Increase the temperature by one and have the LED lights
    ...
    def processTempDecButton(self):
        ...
        ## Decrease the temperature by one and have the LED lights
        ## reflect the current state
        self.setPoint -= 1
        self.updateLights()
    ...
    def updateLights(self):
        ...
        ## Determine visual identifiers based on current state.
        ## Uses PWM (.pulse) for active state and standard digital out (.on) for reached target.
    ...
    def setupSerialOutput(self):
        ## Fetch current temperature and format as State, Temp, SetPoint.
        ## Append \r\n to signal the end of the transmission packet over UART.
        temp = floor(self.getFahrenheit())
    ...
    def manageMyDisplay(self):
        ...
        while not self.endDisplay:
            ...
            ## Formated datetime to string. The \n forces the LCD cursor
            ## down to the next row for the next string.
            lcd_line_1 = current_time.strftime('%b %d %H:%M:%S\n')
    
            ## Setup Display Line 2
            if(altCounter < 6):
                ## Display current temp formatted to one decimal place for UI readability.
                temp = self.getFahrenheit()
                lcd_line_2 = f"Temp: {temp:0.1f} F"
    
                altCounter = altCounter + 1
            else:
                ## Display state and setPoint. This string will stay under the 16 character limit of the LCD.
            ...
            if((counter % 30) == 0):
                ## Fetch formatted string and encode it to UTF-8 Bytes.
                ## Serial ports can't use python strings! They need raw byte transmission!
